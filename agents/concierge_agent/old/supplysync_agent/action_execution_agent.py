...
notification mode and calls the external
Notification MCP.


Valid notification modes are:
...
When notification_mode is:

SMS

The Notification MCP sends an SMS using the
mobile number.
...
The Notification MCP sends an email using the
recipient email.
...
The Notification MCP sends both:

- Email
- SMS


The notification_agent must provide the required
contact details to the Notification MCP, including
when available:
...
Do not create notification data yourself unless
